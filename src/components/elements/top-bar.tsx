...
const TopBar: React.FC<ClassProps> = ({ className }) => {
  ...
  const changeCategory = React.useCallback((target: Element, category: string) => {
    if (parentElement.current && selectionElement.current) {
  ...
  React.useEffect(() => {
    const targetElement = parentElement.current?.querySelector(
      `#${activeCategory || categories[0]}-top-bar`
    );
  ...
  return (
    ...
    >
      <div className="container py-6 flex-space-between">
        <div
          className="relative inline-flex select-none rounded-2xl bg-gray-200 gap-1.5 px-1.5 py-1.5"
          ref={parentElement}
        >
