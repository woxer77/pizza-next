  ...
  return (
    <div className={cn('w-[250px] flex-shrink-0', className)}>
      <div className="sticky top-[140px] h-[calc(100vh-140px)] overflow-y-auto p-1 scroll filterScroll">
        ...
        <Filters
          elements={ingredients.map(e => ({ id: toSlug(e), text: e }))}
          title="Ingredients"
        />{' '}
        {/* temp */}
        <Filters
          elements={doughTypes.map(e => ({ id: toSlug(e), text: e }))}
          title="Baking dough type"
        />{' '}
        {/* temp */}
        <Button className="w-11/12 font-bold py-2 mx-auto">Accept</Button>
      </div>
    </div>
  );
};
